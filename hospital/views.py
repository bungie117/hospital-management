...
def render_to_pdf(request, context):
    ...
    p.drawString(200, y, "Hospital Bill")
    y -= 40

    p.setFont("Helvetica", 12)

    p.drawString(50, y, f"Patient Name: {context['patientName']}")
    y -= 20

    p.drawString(50, y, f"Doctor Name: {context['assignedDoctorName']}")
    y -= 20

    p.drawString(50, y, f"Mobile: {context['mobile']}")
    y -= 20

    p.drawString(50, y, f"Address: {context['address']}")
    y -= 20

    p.drawString(50, y, f"Symptoms: {context['symptoms']}")
    y -= 30

    p.drawString(50, y, f"Admit Date: {context['admitDate']}")
    y -= 20

    p.drawString(50, y, f"Release Date: {context['releaseDate']}")
    y -= 20

    p.drawString(50, y, f"Days Spent: {context['daySpent']}")
    y -= 30

    p.setFont("Helvetica", 12)
    p.drawString(50, y, "Charges:")
    y -= 20

    p.setFont("Helvetica", 12)
    p.drawString(50, y, f"Room Charge: {context['roomCharge']}")
    y -= 20

    p.drawString(50, y, f"Doctor Fee: {context['doctorFee']}")
    y -= 20

    p.drawString(50, y, f"Medicine Cost: {context['medicineCost']}")
    y -= 20

    p.drawString(50, y, f"Other Charge: {context['OtherCharge']}")
    y -= 30

    p.setFont("Helvetica", 14)
    p.drawString(50, y, f"Total: {context['total']}")
...
def download_pdf_view(request, pk):
    dischargeDetails = models.PatientDischargeDetails.objects.filter(
        patientId=pk
    ).order_by('-id')
